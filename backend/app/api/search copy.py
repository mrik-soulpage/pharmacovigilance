        ...
        try:
            ...
            for idx, pmid in enumerate(pmids):
                try:
                    ...
                    # resp = requests.get(f"https://pubmed.ncbi.nlm.nih.gov/{pmid}/", headers={"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36"}, timeout=30)
                    # soup = BeautifulSoup(resp.text, "html.parser")

                    # pmc_href = [txt.get("src") for txt in soup.find_all("a") if txt.get("a") and "pmc.ncbi.nlm.nih.gov" in txt.get("href")]
